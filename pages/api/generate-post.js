...
export default withApiAuthRequired(async function handler(req, res) {
  ...
  if (!userProfile?.availableTokens) {
    return res.status(403).json({
      error: "You do not have enough tokens to generate a blog post.",
    });
  }
  ...
  // const response = await openai.createcCompletion({
  //   model: "",
  //   temperature: 0,
  //   max_tokens: 3600,
  ...
  const titleResponse = await openai.createChatCompletion({
    ...
    messages: [
      ...
      {
        role: "user",
        content: `Generate appropriate title tag text for the above blog post. Just give me text, no html tags.`,
      },
    ],
  });
  ...
  const metaDescriptionResponse = await openai.createChatCompletion({
    ...
    messages: [
      ...
      {
        role: "user",
        content: `Generate SEO-friendly meta description content for the above blog post. Just give me text, no html tags.`,
      },
    ],
  });
